...
/// @brief 重启设备，此函数调用后，设备会断电，重新启动(不是所有机型都支持，KM25/K15D/CMU970/CTG970之后研发的机型支持)
...
/// 
/// @param byTargetFormat:采集图像目标格式设置
...
/// @brief sensor i2c外部上拉电阻选择(CMU958PLUS/DMU927P/CMU959P支持)
/// 
...
/// @param uRegAddrSize：寄存器地址的字节数（不超过255字节）
/// @param pData：读到寄存器的值
/// @param uSize：读出寄存器的数据块的字节数（不超过255字节）
/// @param bNoStop：是否发出I2C的STOP命令，一般情况下默认为FALSE，bNoStop=TRUE表示写的过程不会有I2C的停止命令，bNoStop=FALSE有I2C的停止命令
///
...
/// @param MaxCount：设置要获取的包ID的最大数，DMU956最大64，即PackID的数组大小
///
/// @retval DT_ERROR_OK：设置成功
/// @retval DT_ERROR_FAILD：设置失败
/// @retval DT_ERROR_COMM_ERROR：通讯错误
...
/// @brief 图像通道使能
/// 
/// @param byGrabChannelEnabel：通道选择，A/B通道，参见宏定义“多SENSOR模组通道定义”
/// 
/// @retval DT_ERROR_OK：设置成功
/// @retval DT_ERROR_FAILED：设置失败
/// @retval DT_ERROR_COMM_ERROR：通讯错误
...
/// @brief SPI控制器接口，支持三线或四线通讯，支持单字节传输和多字节连续传输，支持读写要求（仅UT300/UV910/UF920支持）。
///
...
/// @brief 图像抓拍设置，设置使能和一次抓拍的帧数,最大支持255(不建议客户使用，hold和restar功能替代)。
///
...
/// @brief 启动一次抓拍(不建议客户使用，hold和restar功能替代)。
///
...
/// @param uFrameNum:设置重启采集后采集帧数，达到设置的采集帧数，则自动GrabHold,为0的话是连续输出帧
/// 
/// @retval DT_ERROR_OK：设置成功
/// @retval DT_ERROR_FAILD：设置失败
/// @retval DT_ERROR_COMM_ERROR：通讯错误
...
/// @brief 新的抓帧接口，比原采集接口效率更高，使用该接口，用户无需申请buffer
...
/// @retval DT_ERROR_FAILD：获取Ocp状态成功失败
/// @retval DT_ERROR_COMM_ERROR：通讯错误
/// @retval DT_ERROR_PARAMETER_OUT_OF_BOUND：参数超出了范围
///
/// @see SENSOR_POWER
